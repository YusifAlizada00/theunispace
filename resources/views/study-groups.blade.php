...
<x-app-layout>
    ...
    <div x-data="{ showJoinedGroups: false }" class="min-h-screen bg-gray-50/50 py-12 px-4 md:pl-[100px] lg:pl-[300px]">
        <div class="max-w-6xl mx-auto">
            ...
            @if(session('success'))
                <div id="success-msg" class="mb-6 bg-green-100 border border-green-200 text-green-700 px-4 py-3 rounded-lg relative transition-opacity duration-500 ease-out" role="alert">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                ...
                <div class="flex items-center gap-3">
                    {{-- NEW: Joined Groups Button --}}
                    <button @click="showJoinedGroups = true"
                            class="inline-flex items-center justify-center gap-2 bg-white text-slate-700 border border-gray-200 hover:bg-gray-50 hover:text-indigo-600 px-6 py-3 rounded-2xl font-bold shadow-sm transition-all hover:-translate-y-1">
                    ...
                    <a href="{{ route('study-groups.create') }}"
                       class="inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-indigo-200 transition-all hover:-translate-y-1">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                @foreach($studyGroups as $index => $studyGroup)
                    ...
                    <div class="group bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 hover:-translate-y-1 flex flex-col h-full">
                        ...
                        <div class="p-6 pb-4">
                            ...
                            <a href="{{ route('study-groups.show', $studyGroup->slug) }}">
                                <h3 class="text-xl font-bold text-slate-900 leading-tight mb-2 group-hover:text-indigo-600 transition-colors truncate">
                                    {{ $studyGroup->subject }}
                                </h3>
                            </a>

                            <div class="flex items-center gap-2 mb-6">
                                <div class="w-6 h-6 rounded-full bg-slate-100 border border-white shadow-sm overflow-hidden">
                                    <img src="{{ $studyGroup->leader->profile_photo_url }}" class="w-full h-full object-cover">
                                </div>
                                <span class="text-sm text-slate-500 font-medium truncate">
                                    Hosted by
                                    <a href="{{ route('posts.all', ['name' => $studyGroup->leader->slug]) }}" class="hover:underline">
                            ...
                            <div class="space-y-3 bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                {{-- Date --}}
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center text-indigo-500 shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-semibold text-slate-700">{{ \Carbon\Carbon::parse($studyGroup->date)->format('D, M jS') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-semibold text-slate-700">{{ $studyGroup->start_time->format('g:ia') }} - {{ $studyGroup->end_time->format('g:ia') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-sm font-semibold text-slate-700 truncate">{{ $studyGroup->location }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        {{-- JOINED GROUPS MODAL --}}
        <div x-show="showJoinedGroups" style="display: none;" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            {{-- Backdrop --}}
            <div x-show="showJoinedGroups"
                ...
                <div class="flex min-h-full items-center justify-center p-4 text-center">
                    ...
                    <div x-show="showJoinedGroups" @click.outside="showJoinedGroups = false"
                                    ...
                                    @if($myGroups && count($myGroups) > 0)
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            @foreach($myGroups as $group)
                                                <a href="{{ route('study-groups.show', $group->slug) }}" class="flex flex-col bg-gray-50 border border-gray-100 rounded-xl p-4 hover:border-indigo-200 hover:bg-indigo-50/50 hover:shadow-md transition-all group">
                                                    <div class="flex items-center justify-between mb-2">
                                                        <span class="inline-flex items-center justify-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-medium text-indigo-700">
                                                            {{ $group->subject }}
                                                        </span>
                                                        <svg class="h-5 w-5 text-gray-300 group-hover:text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                                                        </svg>
                                                    </div>
                                                    <h4 class="text-lg font-bold text-gray-900 group-hover:text-indigo-700 mb-1">{{ $group->group_name }}</h4>
                                                    <p class="text-xs text-gray-500">Joined {{ $group->pivot->created_at->diffForHumans() }}</p>
                                                </a>
                                            @endforeach
                                        </div>
                                    @else
                        ...
                        <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse">
                            <button type="button" @click="showJoinedGroups = false" class="inline-flex w-full justify-center rounded-xl bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:w-auto sm:py-2">Close</button>
                        </div>
                    </div>
                </div>
            </div>
