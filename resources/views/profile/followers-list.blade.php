        ...
        @foreach ($followers as $follower)
            <div
                ...
                <div class="flex flex-row gap-3 items-center flex-1 min-w-0">
                    <img src="{{ asset($follower->profile_photo_url) }}" loading="lazy" alt="Profile" class="rounded-full w-12 h-12 flex-shrink-0">
                    <div class="flex flex-col overflow-hidden">
                        <span class="font-semibold text-gray-800 truncate">
                            {{ Str::limit($follower->name, 15) }}
                        </span>
                    </div>
                </div>
                <div class="flex gap-2 flex-shrink-0">
                    <a href="{{ route('posts.all', ['name' => $follower->slug]) }}"
