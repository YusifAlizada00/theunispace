@if(config('commentify.comment_nesting') === true)
    @auth
        @if($comment->isParent())
            <button type="button" wire:click="$toggle('isReplying')"
                ...
                {{ __('commentify::commentify.comments.reply') }}
            </button>
    ...
    @if($comment->children->count())
        <button type="button" wire:click="$toggle('hasReplies')"
                class="flex items-center text-sm text-gray-500 hover:underline dark:text-gray-400 whitespace-nowrap">
            @if(!$hasReplies)
                {{ __('commentify::commentify.comments.view_replies', ['count' => $comment->children->count()]) }}
            @else
                {{ __('commentify::commentify.comments.hide_replies') }}
            @endif
        </button>
