            ...
            <footer class="flex justify-between items-center mb-1">
                <div class="flex items-center space-x-2 text-sm text-gray-900 dark:text-gray-800">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        <time pubdate datetime="{{ $comment->getTimeAgoAttribute() }}" title="{{ $comment->presenter()->relativeCreatedAt() }}">
                            {{ $comment->getTimeAgoAttribute() }}
                        </time>
                    </p>
                </div>

                <div class="relative ml-2">
                    <button wire:click="$toggle('showOptions')" class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-400 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-50 dark:bg-white dark:hover:bg-gray-100 dark:focus:ring-white" type="button">
                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"     Box="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
